...
class ToolCenter:
    ...
    async def execute(self, tool_name: str, command: str,
        ...
        if health and not health.is_healthy:
            alt = self._find_healthy_alternative(tool_name)
            if alt:
                logger.info(f"Tool '{tool_name}' unhealthy, switching to '{alt}'")
                tool_name = alt
    ...
    def _find_healthy_alternative(self, tool_name: str) -> Optional[str]:
        ...
        # 同类工具映射
        alternatives = {
            "shell_exec": ["shell_safe", "python_exec"],
            "http_fetch": ["browser_fetch"],
            "browser_fetch": ["http_fetch"],
            "file_read": ["file_list"],
            "file_write": [],
            "python_exec": ["shell_exec"],
        }
        candidates = alternatives.get(tool_name, [])
        for alt in candidates:
            alt_health = self.health.get(alt)
            if alt_health and alt_health.is_healthy:
                cfg = self.configs.get(alt)
