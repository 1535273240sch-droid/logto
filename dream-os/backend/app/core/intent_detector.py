...
def detect_intent(text: str) -> IntentResult:
    ...
    if best_confidence < 0.6:
        fallback = _fallback_intent(text_clean)
        if fallback == "dev" and best_confidence < 0.5:
            best_intent = IntentType.CODE
            best_confidence = 0.4
        elif text_clean.endswith("?") or text_clean.endswith("？"):
...
TECH_KEYWORDS = {
    "代码", "编程", "写", "实现", "bug", "修复", "重构", "部署",
    "docker", "nginx", "api", "数据库", "端口", "日志", "配置",
    "函数", "类", "模块", "import", "npm", "pip", "git", "json",
    "html", "css", "js", "python", "java", "go", "rust", "ts",
    "编译", "构建", "测试", "调试", "优化", "性能", "内存",
    "linux", "bash", "shell", "服务器", "进程", "线程", "缓存",
    "前端", "后端", "框架", "库", "包", "依赖", "版本",
}

def _fallback_intent(text: str) -> str:
    """启发式兜底，不调模型，零延迟"""
    if len(text) > 50 and any(kw in text for kw in TECH_KEYWORDS):
        return "dev"
    if "?" in text:
        return "chat"
    if any(path in text for path in ["/", "\\", "."]):
        return "dev"
    return "chat"  # 默认
