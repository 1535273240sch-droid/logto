...
class BaseAgent(ABC):
    ...
    def effective_max_tokens(self) -> int:
        """按 Agent 角色差异化 max_tokens"""
        short_output_agents = {"coder", "executor", "reviewer", "tester"}
        if self.role in short_output_agents:
            return 1024
    ...
    async def run(
        ...
        self._task_id = blackboard.project_id
        try:
            # 获取 LLM 客户端
    ...
    async def _execute_tool(
        ...
        # V3-fix: 修复 LLM 输出中字面量换行符问题
        if tool_name == "file_read" and arguments.get("command", "").startswith("write:"):
            raw_cmd = arguments["command"]
            # 检测字面量 \n (反斜杠+n两个字符) 且没有真实换行符
            literal_n = "\\n"
            if literal_n in raw_cmd and "\n" not in raw_cmd:
                arguments["command"] = raw_cmd.replace(literal_n, "\n")
                logger.info(f"[{self.role}] Fixed literal newline in write command")
        """执行单个工具调用 (带安全检查)"""
        ...
        if tool_name in ("shell_exec", "shell_safe"):
            ...
            if risk == RiskLevel.DANGEROUS:
                ...
                _confirmation_states[task_id] = {"event": confirm_event, "action": None}

                await emit({
                    "type": "human_checkpoint",
                    "checkpoint_type": "dangerous_command",
                    "task_id": task_id,
                    "agent": self.role,
                    "tool": tool_name,
                    "command": command,
                ...
                try:
                    ...
                    _confirmation_states.pop(task_id, None)
                    return {"success": False, "output": f"⏸️ 等待确认超时，操作已取消: {reason}"}

                action = _confirmation_states.pop(task_id, {}).get("action")
                if action != "proceed":
                    return {"success": False, "output": f"⏸️ 用户取消了操作: {reason}"}
