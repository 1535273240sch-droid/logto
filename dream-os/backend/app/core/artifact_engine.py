    ...
    def _restore_from_disk(self):
        ...
        try:
            ...
            for f in ARTIFACT_DIR.iterdir():
                ...
                ext = f.suffix.lower()
                type_map = {
                    '.md': 'markdown', '.txt': 'txt', '.csv': 'csv',
                    '.xlsx': 'excel', '.pdf': 'pdf', '.pptx': 'ppt',
                    '.docx': 'word', '.html': 'html',
                    '.mmd': 'mermaid', '.svg': 'bar_chart',
                    '.py': 'code', '.json': 'api_doc',
                }
                artifact_type = type_map.get(ext, 'unknown')
                title = parts[1].rsplit('.', 1)[0] if '.' in parts[1] else parts[1]
                artifact = Artifact(
                    id=artifact_id,
                    filename=name,
                ...
                count += 1
            if count > 0:
                logger.info(f"Restored {count} artifacts from disk")
        except Exception as e:
            logger.warning(f"Failed to restore artifacts from disk: {e}")
